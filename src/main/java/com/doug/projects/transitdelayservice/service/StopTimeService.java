...
public class StopTimeService {
    ...
    public List<String> getScheduledDepartureTimesForStop(String stopName, String routeName) {
        ...
        return stopTimeRepository.getStopTimes(stopIds, tripIds).map(StopTime::getDeparture_time).sequential()
                .unordered().distinct().sorted(Comparator.comparing(StopTimeService::parseTime)).toList();
    }
    ...
    public List<Integer> getTenNearestStopsFor(int tripId, int stopId, int stopRangeToCheck) {
        List<Integer> stopTimes =
                stopTimeRepository.getStopTimes(tripId).sorted(Comparator.comparing(StopTime::getStop_sequence))
                        .map(StopTime::getStop_id).collect(Collectors.toList());
